	...
	kp2m.addEventListener("keydown", function(event) {
		...
		if(kp2m.value == "" && event.keyCode == 8) {
			if(kp2m.previousElementSibling) {
				if(kp2m.previousElementSibling.getAttribute("id") == "pm_simpleOrder_day") dateDay = true;
				else if(kp2m.previousElementSibling.getAttribute("id") == "pm_simpleOrder_time") dateTime = true;
